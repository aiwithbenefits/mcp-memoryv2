...
export class MyMCP extends McpAgent<Env, {}, MyMCPProps> {
  ...
  async init() {
    ...
    this.server.tool(
      "ingestEmail",
      `Stores a new email in the memory system. Use this when an email is received to capture all relevant metadata and content for future retrieval and analysis.`,
      {
      ...
      async (emailData) => {
        try {
          // Store in Vectorize with enhanced metadata
          ...
          console.log(`Email ingested successfully with ID: ${memoryId}`);

          return {
            content: [{
              type: "text",
              text: `Email stored successfully:\nFrom: ${emailData.senderName || emailData.senderEmail}\nSubject: ${emailData.subject || '(no subject)'}\nDate: ${emailData.dateSent}\nID: ${memoryId}`
            }],
          };
        } catch (error) {
    ...
    // Enhanced email search tool
    this.server.tool(
      "searchEmailMemory",
      `Searches stored emails using semantic search with optional filters. This tool understands context and can find emails even when exact keywords don't match.`,
      {
      ...
      async (searchParams) => {
        try {
          console.log(`Searching emails with query: "${searchParams.query}"`);
...
${m.content.substring(0, 200)}${m.content.length > 200 ? '...' : ''}`;
            ...
            return {
              content: [{
                type: "text",
                text: `Found ${memories.length} emails:\n\n${results}`
              }],
            };
          }
...
Date range: ${emails[emails.length-1].dateSent} to ${emails[0].dateSent}`;
    ...
    this.server.tool(
      "findSimilarEmails",
      `Finds emails similar to a given email based on content, context, and metadata. Useful for finding related communications.`,
      {
...
ID: ${e.id}`;
          ...
          return {
            content: [{
              type: "text",
              text: `Found ${similarEmails.length} similar emails:\n\n${results}`
            }],
          };
        } catch (error) {
    ...
    this.server.tool(
      "addToMCPMemory",
      `Stores important information in memory. For emails, use ingestEmail instead. This tool is for general notes and information.`,
    ...
    this.server.tool(
      "searchMCPMemory",
      `Searches all stored information including emails and notes. For email-specific searches, use searchEmailMemory for better results.`,
          ...
          if (memories.length > 0) {
            const results = memories.map(m => {
              ...
              if (meta.emailType === 'note' || meta.senderEmail === 'notes@system.internal') {
                return `${m.content} (score: ${m.score.toFixed(4)})`;
              } else {
                return `Email from ${meta.senderName || meta.senderEmail}: ${meta.subject || '(no subject)'}\n${m.content.substring(0, 100)}... (score: ${m.score.toFixed(4)})`;
              }
            }).join('\n\n');
